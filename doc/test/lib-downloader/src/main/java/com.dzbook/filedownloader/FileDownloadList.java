...
public class FileDownloadList {
    ...
    private final ArrayList<com.dzbook.filedownloader.BaseDownloadTask.IRunningTask> mList;
    ...
    int count(final int id) {
        int size = 0;
        synchronized (mList) {
            for (com.dzbook.filedownloader.BaseDownloadTask.IRunningTask task : mList) {
                if (task.is(id)) {
                    size++;
                }
            }
     ...
     * @return IRunningTask
     */
    public com.dzbook.filedownloader.BaseDownloadTask.IRunningTask get(final int id) {
        synchronized (mList) {
            for (com.dzbook.filedownloader.BaseDownloadTask.IRunningTask task : mList) {
    ...
    List<com.dzbook.filedownloader.BaseDownloadTask.IRunningTask> getReceiveServiceTaskList(final int id) {
        final List<com.dzbook.filedownloader.BaseDownloadTask.IRunningTask> list = new ArrayList<>();
        synchronized (this.mList) {
            for (com.dzbook.filedownloader.BaseDownloadTask.IRunningTask task : this.mList) {
                if (task.is(id) && !task.isOver()) {

                    final byte status = task.getOrigin().getStatus();
                    if (status != FileDownloadStatus.INVALID_STATUS
                            && status != FileDownloadStatus.TO_LAUNCH_POOL) {
                        list.add(task);
                    }
                }
            }
    ...
    List<com.dzbook.filedownloader.BaseDownloadTask.IRunningTask> getDownloadingList(final int id) {
        final List<com.dzbook.filedownloader.BaseDownloadTask.IRunningTask> list = new ArrayList<>();
        synchronized (this.mList) {
            for (com.dzbook.filedownloader.BaseDownloadTask.IRunningTask task : this.mList) {
                if (task.is(id)
    ...
    boolean isNotContains(final com.dzbook.filedownloader.BaseDownloadTask.IRunningTask download) {
        return mList.isEmpty() || !mList.contains(download);
    }

    List<com.dzbook.filedownloader.BaseDownloadTask.IRunningTask> copy(final FileDownloadListener listener) {
        final List<com.dzbook.filedownloader.BaseDownloadTask.IRunningTask> targetList = new ArrayList<>();
        synchronized (mList) {
    ...
    List<com.dzbook.filedownloader.BaseDownloadTask.IRunningTask> assembleTasksToStart(int attachKey,
    ...
    com.dzbook.filedownloader.BaseDownloadTask.IRunningTask[] copy() {
        synchronized (mList) {
            // Prevent size changing
            com.dzbook.filedownloader.BaseDownloadTask.IRunningTask[] copy = new com.dzbook.filedownloader.BaseDownloadTask.IRunningTask[mList.size()];
    ...
    void divertAndIgnoreDuplicate(
            @SuppressWarnings("SameParameterValue") final List<com.dzbook.filedownloader.BaseDownloadTask.IRunningTask>
                    destination) {
        synchronized (mList) {
            for (com.dzbook.filedownloader.BaseDownloadTask.IRunningTask iRunningTask : mList) {
     ...
     * @return boolean
     */
    public boolean remove(final com.dzbook.filedownloader.BaseDownloadTask.IRunningTask willRemoveDownload,
    ...
    void add(final com.dzbook.filedownloader.BaseDownloadTask.IRunningTask task) {
     ...
     *
     * @see com.dzbook.filedownloader.BaseDownloadTask.InQueueTask#enqueue()
     */
    void addUnchecked(final com.dzbook.filedownloader.BaseDownloadTask.IRunningTask task) {
