...
class QueuesHandler implements IQueuesHandler {
    ...
    public boolean startQueueSerial(FileDownloadListener listener) {
        ...
        final HandlerThread serialThread = new HandlerThread(FileDownloadUtils.formatString("filedownloader serial thread %s-%d", listener, attachKey));
        serialThread.start();
    ...
    private boolean onAssembledTasksToStart(int attachKey, final List<BaseDownloadTask.IRunningTask> list, final FileDownloadListener listener, boolean isSerial) {
    ...
    private class SerialHandlerCallback implements Handler.Callback {
        ...
        SerialHandlerCallback() {
            mSerialFinishListener = new SerialFinishListener(new WeakReference<>(this));
        }
        ...
        public boolean handleMessage(final Message msg) {
            if (msg.what == WHAT_SERIAL_NEXT) {
                ...
                synchronized (stackTopTask.getPauseLock()) {
                    if (stackTopTask.getOrigin().getStatus() != FileDownloadStatus.INVALID_STATUS || FileDownloadList.getImpl().isNotContains(stackTopTask)) {
                        // pause?
                        if (FileDownloadLog.NEED_LOG) {
                    ...
                    stackTopTask.getOrigin().addFinishListener(mSerialFinishListener.setNextIndex(mRunningIndex + 1));
