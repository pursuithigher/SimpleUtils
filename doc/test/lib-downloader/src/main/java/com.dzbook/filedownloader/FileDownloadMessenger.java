...
/**
 * The messenger for sending messages to  {@link com.dzbook.filedownloader.FileDownloadListener}.
 *
 * @see IFileDownloadMessenger
 */
class FileDownloadMessenger implements IFileDownloadMessenger {

    private com.dzbook.filedownloader.BaseDownloadTask.IRunningTask mTask;
    private com.dzbook.filedownloader.BaseDownloadTask.LifeCycleCallback mLifeCycleCallback;

    private Queue<MessageSnapshot> parcelQueue;

    private boolean mIsDiscard = false;

    FileDownloadMessenger(final com.dzbook.filedownloader.BaseDownloadTask.IRunningTask task,
                          final com.dzbook.filedownloader.BaseDownloadTask.LifeCycleCallback callback) {
        init(task, callback);
    }

    private void init(final com.dzbook.filedownloader.BaseDownloadTask.IRunningTask task,
                      com.dzbook.filedownloader.BaseDownloadTask.LifeCycleCallback callback) {
        this.mTask = task;
    ...
    public void notifyProgress(MessageSnapshot snapshot) {
        final com.dzbook.filedownloader.BaseDownloadTask originTask = mTask.getOrigin();
        if (FileDownloadLog.NEED_LOG) {
    ...
    private void process(MessageSnapshot snapshot) {
        ...
        if (mIsDiscard || mTask.getOrigin().getListener() == null) {
            if ((com.dzbook.filedownloader.FileDownloadMonitor.isValid() || mTask.isContainFinishListener())
                    && snapshot.getStatus() == FileDownloadStatus.BLOCK_COMPLETE) {
    ...
    public void handoverMessage() {
        ...
        final com.dzbook.filedownloader.BaseDownloadTask.IRunningTask task = mTask;
        ...
        final com.dzbook.filedownloader.BaseDownloadTask originTask = task.getOrigin();
        ...
        if (currentStatus == FileDownloadStatus.BLOCK_COMPLETE) {
            try {
            ...
            onElse(message, currentStatus, originTask, listener);
        }
        ;
    }

    private void onElse(MessageSnapshot message, int currentStatus, BaseDownloadTask originTask, FileDownloadListener listener) {
        ...
        switch (currentStatus) {
            case FileDownloadStatus.PENDING:
                handlePending(message, originTask, listener, largeFileListener);
            ...
            case FileDownloadStatus.CONNECTED:
                handleConnected(message, originTask, listener, largeFileListener);

                break;
            case FileDownloadStatus.PROGRESS:
                handleProgress(message, originTask, listener, largeFileListener);
                break;
            case FileDownloadStatus.RETRY:
                handleRetry(message, originTask, listener, largeFileListener);
            ...
            case FileDownloadStatus.PAUSED:
                handlePaused(message, originTask, listener, largeFileListener);
                break;
    ...
    private void handlePaused(MessageSnapshot message, BaseDownloadTask originTask, FileDownloadListener listener, FileDownloadLargeFileListener largeFileListener) {
    ...
    private void handleRetry(MessageSnapshot message, BaseDownloadTask originTask, FileDownloadListener listener, FileDownloadLargeFileListener largeFileListener) {
        if (largeFileListener != null) {
    ...
    private void handleProgress(MessageSnapshot message, BaseDownloadTask originTask, FileDownloadListener listener, FileDownloadLargeFileListener largeFileListener) {
        if (largeFileListener != null) {
    ...
    private void handleConnected(MessageSnapshot message, BaseDownloadTask originTask, FileDownloadListener listener, FileDownloadLargeFileListener largeFileListener) {
        if (largeFileListener != null) {
    ...
    private void handlePending(MessageSnapshot message, BaseDownloadTask originTask, FileDownloadListener listener, FileDownloadLargeFileListener largeFileListener) {
        if (largeFileListener != null) {
    ...
    @Override
    public void reAppointment(com.dzbook.filedownloader.BaseDownloadTask.IRunningTask task,
    ...
    public boolean isBlockingCompleted() {
        return parcelQueue.peek().getStatus() == FileDownloadStatus.BLOCK_COMPLETE;
    }
