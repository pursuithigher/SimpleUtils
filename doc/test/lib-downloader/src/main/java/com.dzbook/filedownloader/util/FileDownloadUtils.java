...
import com.dzbook.filedownloader.download.CustomComponentHolder;
...
import com.example.lib_downloader.BuildConfig;
...
public class FileDownloadUtils {
     ...
     * @see com.dzbook.filedownloader.download.DownloadStatusCallback#onProgress(long)
     ...
     * @see com.dzbook.filedownloader.BaseDownloadTask#setPath(String, boolean)
     */
    ...
    public static String getStack(final StackTraceElement[] stackTrace, final boolean printLine) {
        ...
        for (int i = 3; i < stackTrace.length; i++) {
            if (!stackTrace[i].getClassName().contains("com.dzbook.filedownloader")) {
                continue;
            }
            t.append("[");
            t.append(stackTrace[i].getClassName().substring("com.dzbook.filedownloader".length()));
            t.append(":");
    ...
    public static boolean isDownloaderProcess(final Context context) {
        ...
        do {
            ...
            if (activityManager == null) {
                com.dzbook.filedownloader.util.FileDownloadLog.w(FileDownloadUtils.class, "fail to get the activity manager!");
                return false;
            }
            ...
            if (null == runningAppProcessInfoList || runningAppProcessInfoList.isEmpty()) {
                com.dzbook.filedownloader.util.FileDownloadLog.w(FileDownloadUtils.class, "The running app process info list from" + " ActivityManager is null or empty, maybe current App is not " + "running.");
                return false;
            }
     ...
     * convertHeaderString
     *
     * @param nameAndValuesString nameAndValuesString
     * @return String
     */
    ...
    public static void markConverted(final Context context) {
        ...
        try {
            ...
            Log.d(TAG, "mkdirs = " + mkdirs + " newFile=" + newFile);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static long parseContentRangeFoInstanceLength(String contentRange) {
        ...
        if (session.length >= 2) {
            try {
                ...
                com.dzbook.filedownloader.util.FileDownloadLog.w(FileDownloadUtils.class, "parse instance length failed with %s", contentRange);
            }
        }
    ...
    public static boolean isNetworkNotOnWifiType() {
        ...
        if (manager == null) {
            com.dzbook.filedownloader.util.FileDownloadLog.w(FileDownloadUtils.class, "failed to get connectivity manager!");
            return true;
        }
    ...
    public static String findEtag(final int id, FileDownloadConnection connection) {
        ...
        if (com.dzbook.filedownloader.util.FileDownloadLog.NEED_LOG) {
            com.dzbook.filedownloader.util.FileDownloadLog.d(FileDownloadUtils.class, "etag find %s for task(%d)", newEtag, id);
        }
    ...
    public static boolean isAcceptRange(int responseCode, FileDownloadConnection connection) {
        if (responseCode == HttpURLConnection.HTTP_PARTIAL || responseCode == FileDownloadConnection.RESPONSE_CODE_FROM_OFFSET) {
            return true;
        }
    ...
    public static long findInstanceLengthForTrial(FileDownloadConnection connection) {
        ...
        if (length < 0) {
            length = FileDownloadModel.TOTAL_VALUE_IN_CHUNKED_RESOURCE;
            com.dzbook.filedownloader.util.FileDownloadLog.w(FileDownloadUtils.class, "don't get instance length from" + "Content-Range header");
        }
    ...
    public static long findContentLength(final int id, FileDownloadConnection connection) {
        long contentLength = convertContentLengthString(connection.getResponseHeaderField("Content-Length"));
        ...
        if (contentLength < 0) {
            ...
            if (!isEncodingChunked) {
                ...
                if (FileDownloadProperties.getImpl().httpLenient) {
                    ...
                    contentLength = FileDownloadModel.TOTAL_VALUE_IN_CHUNKED_RESOURCE;
                    if (com.dzbook.filedownloader.util.FileDownloadLog.NEED_LOG) {
                        com.dzbook.filedownloader.util.FileDownloadLog.d(FileDownloadUtils.class, "%d response header is not legal but " + "HTTP lenient is true, so handle as the case of " + "transfer encoding chunk", id);
                    }
                } else {
    ...
    public static long parseContentLengthFromContentRange(String contentRange) {
        ...
        try {
            ...
            com.dzbook.filedownloader.util.FileDownloadLog.e(FileDownloadUtils.class, e, "parse content length" + " from content range error");
        }
        return -1;
    }
     ...
     * @param url        url
     * @return String
     * @throws FileDownloadSecurityException FileDownloadSecurityException
     */
     ...
     *
     * @param path path
     * @return FileDownloadOutputStream
     ...
     * @param id                      id
     * @param model                   model
     * @param outputStreamSupportSeek outputStreamSupportSeek
     * @return can resume by break point
     */
    public static boolean isBreakpointAvailable(final int id, final FileDownloadModel model, final Boolean outputStreamSupportSeek) {
        if (model == null) {
            if (com.dzbook.filedownloader.util.FileDownloadLog.NEED_LOG) {
                com.dzbook.filedownloader.util.FileDownloadLog.d(FileDownloadUtils.class, "can't continue %d model == null", id);
            }
            return false;
        }
        ...
        if (model.getTempFilePath() == null) {
            if (com.dzbook.filedownloader.util.FileDownloadLog.NEED_LOG) {
                com.dzbook.filedownloader.util.FileDownloadLog.d(FileDownloadUtils.class, "can't continue %d temp path == null", id);
            }
            return false;
        }
     ...
     * @return boolean
     */
    public static boolean isBreakpointAvailable(final int id, final FileDownloadModel model, final String path, final Boolean outputStreamSupportSeek) {
        boolean result = false;

        do {
            if (path == null) {
                if (com.dzbook.filedownloader.util.FileDownloadLog.NEED_LOG) {
                    com.dzbook.filedownloader.util.FileDownloadLog.d(FileDownloadUtils.class, "can't continue %d path = null", id);
                }
                break;
            }
            ...
            if (!isExists || isDirectory) {
                if (com.dzbook.filedownloader.util.FileDownloadLog.NEED_LOG) {
                    com.dzbook.filedownloader.util.FileDownloadLog.d(FileDownloadUtils.class, "can't continue %d file not suit, exists[%B], directory[%B]", id, isExists, isDirectory);
                }
                break;
            }
            ...
            if (handleModel(id, model, outputStreamSupportSeek, fileLength, currentOffset)) {
                break;
            }
    ...
    private static boolean handleModel(int id, FileDownloadModel model, Boolean outputStreamSupportSeek, long fileLength, long currentOffset) {
        ...
        if (fileLength < currentOffset || (totalLength != FileDownloadModel.TOTAL_VALUE_IN_CHUNKED_RESOURCE  // not chunk transfer
        ...
        if (outputStreamSupportSeek != null && !outputStreamSupportSeek && totalLength == fileLength) {
            if (FileDownloadLog.NEED_LOG) {
     ...
     * isNeedSync
     *
     * @param bytesDelta     bytesDelta
     * @param timestampDelta timestampDelta
     * @return boolean
     */
    public static boolean isNeedSync(long bytesDelta, long timestampDelta) {
        return bytesDelta > FileDownloadUtils.getMinProgressStep() && timestampDelta > FileDownloadUtils.getMinProgressTime();
    }
