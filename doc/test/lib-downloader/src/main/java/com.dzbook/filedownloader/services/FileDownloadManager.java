...
import com.dzbook.filedownloader.download.DownloadRunnable;
...
class FileDownloadManager implements IThreadPoolMonitor {
    ...
    public synchronized void start(final String url, final String path,
        ...
        if (checkIsStart(id, model)) {
            return;
        }

        if (checkFile(path, pathAsDirectory, forceReDownload, id, model)) {
            return;
        }
        ...
        if (checkModel(model)) // FileDownloadRunnable
        // invoke #isBreakpointAvailable to determine whether it is really invalid.
        {
            needUpdate2DB = isNeedUpdate2DB(url, path, pathAsDirectory, id, model, dirConnectionModelList);
        } else {
            if (model == null) {
    ...
    private boolean isNeedUpdate2DB(String url, String path, boolean pathAsDirectory, int id, FileDownloadModel model, List<ConnectionModel> dirConnectionModelList) {
        boolean needUpdate2DB;
        if (model.getId() != id) {
            // in try dir case.
        ...
        return needUpdate2DB;
    }

    private boolean checkFile(String path, boolean pathAsDirectory, boolean forceReDownload, int id, FileDownloadModel model) {
        ...
        if (checkDowloadComplete(forceReDownload, id, targetFilePath)) {
            return true;
        }

        if (checkTempFile(id, model, targetFilePath)) {
            return true;
        }
        return false;
    }

    private boolean checkIsStart(int id, FileDownloadModel model) {
    ...
    private boolean checkModel(FileDownloadModel model) {
        return model != null
    ...
    private boolean checkTempFile(int id, FileDownloadModel model, String targetFilePath) {
    ...
    private boolean checkDowloadComplete(boolean forceReDownload, int id, String targetFilePath) {
