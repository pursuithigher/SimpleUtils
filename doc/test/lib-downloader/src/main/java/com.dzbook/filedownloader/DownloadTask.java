...
public class DownloadTask implements BaseDownloadTask, BaseDownloadTask.IRunningTask, DownloadTaskHunter.ICaptureTask {
    ...
    private static volatile FileDownloadHeader mHeader;

    volatile int mAttachKey = 0;
    private final com.dzbook.filedownloader.ITaskHunter mHunter;
    private final com.dzbook.filedownloader.ITaskHunter.IMessageHandler mMessageHandler;
    private int mId;
    ...
    public boolean reuse() {
        if (isRunning()) {
            FileDownloadLog.w(this, "This task[%d] is running, if you want start the same task," + " please create a new one by FileDownloader#create", getId());
            return false;
        }
    ...
    private int startTaskUnchecked() {
        if (isUsing()) {
            if (isRunning()) {
                throw new IllegalStateException(FileDownloadUtils.formatString("This task is running %d, if you" + " want to start the same task, please create a new one by" + " FileDownloader.create", getId()));
            } else {
                throw new IllegalStateException("This task is dirty to restart, If you want to " + "reuse this task, please invoke #reuse method manually and retry to " + "restart again." + mHunter.toString());
            }
        }
    ...
    @Override
    public com.dzbook.filedownloader.ITaskHunter.IMessageHandler getMessageHandler() {
