...
public class FileDownloadSerialQueue {
    ...
    public FileDownloadSerialQueue() {
        mHandlerThread = new HandlerThread(com.dzbook.filedownloader.util.FileDownloadUtils.getThreadPoolName("SerialDownloadManager"));
    ...
    public void pause() {
        synchronized (finishCallback) {
            if (paused) {
                com.dzbook.filedownloader.util.FileDownloadLog.w(this, "require pause this queue(remain %d), but " + "it has already been paused", mTasks.size());
                return;
            }
    ...
    public void resume() {
        synchronized (finishCallback) {
            if (!paused) {
                FileDownloadLog.w(this, "require resume this queue(remain %d), but it is" + " still running", mTasks.size());
                return;
            }
            ...
            switch (msg.what) {
                case WHAT_NEXT:
                    try {
                        ...
                        workingTask.addFinishListener(finishCallback).start();
