...
class FileDownloadThreadPool {
    private static final String THREAD_PREFIX = "Network";
    ...
    public synchronized void execute(DownloadLaunchRunnable launchRunnable) {
    ...
    public synchronized boolean isInThreadPool(final int downloadId) {
        DownloadLaunchRunnable runnable = runnablePool.get(downloadId);
