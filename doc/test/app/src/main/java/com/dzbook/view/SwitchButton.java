...
public class SwitchButton extends View implements Checkable {
     ...
     * 6.点击切换
     **/
    private static final int M_ANIMATESTATENONE = 0;
    private static final int M_ANIMATE_STATE_PENDING_DRAG = 1;
    private static final int M_ANIMATE_STATE_DRAGING = 2;
    private static final int M_ANIMATE_STATE_PENDING_RESET = 3;
    private static final int M_ANIMATE_STATE_PENDING_SETTLE = 4;
    private static final int M_ANIMATE_STATE_SWITCH = 5;
    private static final float NUM_3 = 0.5f;
     ...
     * 动画状态
     */
    private int animateState = M_ANIMATESTATENONE;
    ...
    private ValueAnimator.AnimatorUpdateListener animatorUpdateListener = new ValueAnimator.AnimatorUpdateListener() {
        ...
        public void onAnimationUpdate(ValueAnimator animation) {
            ...
            switch (animateState) {
                case M_ANIMATE_STATE_PENDING_SETTLE: {
                }
                case M_ANIMATE_STATE_PENDING_RESET: {
                }
                case M_ANIMATE_STATE_PENDING_DRAG: {
                    ...
                    if (animateState != M_ANIMATE_STATE_PENDING_DRAG) {
                ...
                default:
                case M_ANIMATE_STATE_DRAGING: {
                }
                case M_ANIMATESTATENONE: {
                    break;
                }
            }
            postInvalidate();
    ...
    private Animator.AnimatorListener animatorListener = new Animator.AnimatorListener() {
        ...
        public void onAnimationEnd(Animator animation) {
            switch (animateState) {
                ...
                case M_ANIMATE_STATE_PENDING_DRAG: {
                    animateState = M_ANIMATE_STATE_DRAGING;
                ...
                case M_ANIMATE_STATE_PENDING_RESET: {
                    animateState = M_ANIMATESTATENONE;
                    postInvalidate();
                    break;
                }
                case M_ANIMATE_STATE_PENDING_SETTLE: {
    ...
    private void init(Context context, AttributeSet attrs) {
        final float num1 = 1.5f;
        final float num2 = 2.5f;
        ...
        uncheckCircleWidth = optPixelSize(typedArray, R.styleable.SwitchButton_sb_uncheckcircle_width, dp2pxInt(num1));//dp2pxInt(1.5f);
        ...
        shadowRadius = optPixelSize(typedArray, R.styleable.SwitchButton_sb_shadow_radius, dp2pxInt(num2));//dp2pxInt(2.5f);

        shadowOffset = optPixelSize(typedArray, R.styleable.SwitchButton_sb_shadow_offset, dp2pxInt(num1));//dp2pxInt(1.5f);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        viewRadius = height * NUM_3;
        buttonRadius = viewRadius - borderWidth;

        left = (getMeasuredWidth() - w) / 2f;
        top = (getMeasuredHeight() - h) / 2f;
        right = left + width;
        bottom = top + height;

        centerY = (top + bottom) * NUM_3;
     ...
     */
    private void setUncheckViewState(ViewState state) {
        state.radius = 0;
        state.checkStateColor = uncheckColor;
        state.checkedLineColor = Color.TRANSPARENT;
        state.buttonX = buttonMinX;
    }
     ...
     * @param state viewState
     */
    private void setCheckedViewState(ViewState state) {
        state.radius = viewRadius;
        state.checkStateColor = checkedColor;
        state.checkedLineColor = checkLineColor;
        state.buttonX = buttonMaxX;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //绘制开启背景色
        float des = viewState.radius * NUM_3;//[0-backgroundRadius*0.5f]
        paint.setStyle(Paint.Style.STROKE);
        paint.setColor(viewState.checkStateColor);
        paint.setStrokeWidth(borderWidth + des * num);
     ...
     * @param p         paint
     */
    protected void drawCheckedIndicator(Canvas canvas, int color, float lineWidth, float sx, float sy, float ex, float ey, Paint p) {
     ...
     * @param color     color
     * @param lineWidth lineWidth
     * @param centerX1  centerX
     * @param centerY1  centerY
     * @param radius    radius
     * @param p         paint
     */
    protected void drawUncheckIndicator(Canvas canvas, int color, float lineWidth, float centerX1, float centerY1, float radius, Paint p) {
        p.setStyle(Paint.Style.STROKE);
        p.setColor(color);
        p.setStrokeWidth(lineWidth);
        canvas.drawCircle(centerX1, centerY1, radius, p);
    }
     ...
     * @param paint1     paint
     */
    private void drawArc(Canvas canvas, float l, float t, float r, float b, float startAngle, float sweepAngle, Paint paint1) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            canvas.drawArc(l, t, r, b, startAngle, sweepAngle, true, paint1);
        } else {
            rect.set(l, t, r, b);
     ...
     * @param r                r
     * @param b                b
     * @param backgroundRadius backgroundRadius
     * @param p                p
     */
    private void drawRoundRect(Canvas canvas, float l, float t, float r, float b, float backgroundRadius, Paint p) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            canvas.drawRoundRect(l, t, r, b, backgroundRadius, backgroundRadius, p);
        } else {
            rect.set(l, t, r, b);
            canvas.drawRoundRect(rect, backgroundRadius, backgroundRadius, p);
        }
    }
    ...
    private void toggle(boolean animate, boolean broadcast) {
        ...
        animateState = M_ANIMATE_STATE_SWITCH;
        ...
        switch (actionMasked) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (System.currentTimeMillis() - touchDownTime <= 300) {
                    ...
                    float fraction = eventX / getWidth();
                    fraction = Math.max(0f, Math.min(1f, fraction));
                    boolean newCheck = fraction > NUM_3;
    ...
    private boolean isInAnimating() {
        return animateState != M_ANIMATESTATENONE;
    }
    ...
    private boolean isPendingDragState() {
        return animateState == M_ANIMATE_STATE_PENDING_DRAG || animateState == M_ANIMATE_STATE_PENDING_RESET;
    }
    ...
    private boolean isDragState() {
        return animateState == M_ANIMATE_STATE_DRAGING;
    }
    ...
    private void pendingDragState() {
        ...
        animateState = M_ANIMATE_STATE_PENDING_DRAG;
    ...
    private void pendingCancelDragState() {
        if (isDragState() || isPendingDragState()) {
            ...
            animateState = M_ANIMATE_STATE_PENDING_RESET;
            beforeState.copy(viewState);
    ...
    private void pendingSettleState() {
        ...
        animateState = M_ANIMATE_STATE_PENDING_SETTLE;
        beforeState.copy(viewState);
