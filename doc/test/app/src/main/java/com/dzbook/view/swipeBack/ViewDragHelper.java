...
public class ViewDragHelper {
    ...
    private ViewDragHelper(Context context, ViewGroup forParent, Callback cb) {
        ...
        mScroller = new OverScroller(context, S_INTERPOLATOR);
    }
    ...
    public boolean settleCapturedViewAt(int finalLeft, int finalTop) {
        ...
        return forceSettleCapturedViewAt(finalLeft, finalTop, (int) mVelocityTracker.getXVelocity(mActivePointerId), (int) mVelocityTracker.getYVelocity(mActivePointerId));
    }
    ...
    public void flingCapturedView(int minLeft, int minTop, int maxLeft, int maxTop) {
        ...
        mScroller.fling(mCapturedView.getLeft(), mCapturedView.getTop(), (int) mVelocityTracker.getXVelocity(mActivePointerId), (int) mVelocityTracker.getYVelocity(mActivePointerId), minLeft, maxLeft, minTop, maxTop);
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN: {
                doActionPointDown(ev, actionIndex);
                break;
            }
    ...
    private void doActionPointDown(MotionEvent ev, int actionIndex) {
        final int pointerId = ev.getPointerId(actionIndex);
    ...
    public void processTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN: {
                processActionPointerDown(ev, actionIndex);
                break;
            }

            case MotionEvent.ACTION_MOVE: {
                processActionMove(ev);
                break;
            }

            case MotionEvent.ACTION_POINTER_UP: {
                processActionPointerUp(ev, actionIndex);
                break;
            }
    ...
    private void processActionDown(MotionEvent ev) {
        final float x = ev.getX();
        final float y = ev.getY();
        final int pointerId = ev.getPointerId(0);
        final View toCapture = findTopChildUnder((int) x, (int) y);
    ...
    private void processActionPointerDown(MotionEvent ev, int actionIndex) {
        final int pointerId = ev.getPointerId(actionIndex);
        final float x = ev.getX(actionIndex);
        final float y = ev.getY(actionIndex);
    ...
    private void processActionMove(MotionEvent ev) {
        if (mDragState == STATE_DRAGGING) {
    ...
    private void processActionPointerUp(MotionEvent ev, int actionIndex) {
        final int pointerId = ev.getPointerId(actionIndex);
    ...
    private boolean checkNewEdgeDrag(float delta, float oDelta, int pointerId, int edge) {
        ...
        if ((mInitialEdgeTouched[pointerId] & edge) != edge || (mTrackingEdges & edge) == 0) {
            return false;
        } else if ((mEdgeDragsLocked[pointerId] & edge) == edge || (mEdgeDragsInProgress[pointerId] & edge) == edge) {
            return false;
        } else if (absDelta <= mTouchSlop && absODelta <= mTouchSlop) {
            return false;
        }
