...
/**
 * 带Loading 状态的PersonCommonView, 因为其Loading 切换问题，这里提出来单独处理（至于他引起的维护麻烦，请评估）
 *
...
public class PersonCommonLoadingView extends RelativeLayout {
    ...
    private HwProgressBar mRightLoading;
     ...
     * @param attrs   attrs
     */
    public PersonCommonLoadingView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        initView(attrs);
        initData();
    ...
    private void initView(AttributeSet attrs) {
        ...
        View view = LayoutInflater.from(mContext).inflate(R.layout.view_person_common_load, this);
        mRightIcon = view.findViewById(R.id.imageview_jiantou);
        mRightLoading = view.findViewById(R.id.imageview_loading_view);
    ...
    public synchronized void showRightIcon() {
        if (mRightIcon != null && mRightLoading != null) {
            mRightIcon.setVisibility(VISIBLE);
            mRightLoading.setVisibility(GONE);
        }
    }
    ...
    public synchronized void showRightLoading() {
        if (mRightIcon != null && mRightLoading != null) {
            ...
            mRightLoading.setVisibility(VISIBLE);
        }
    }
}
