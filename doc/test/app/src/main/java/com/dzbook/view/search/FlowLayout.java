...
public class FlowLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < childVisibleCount; i++) {
            ...
            if (i == childVisibleCount - 1) {
                width = Math.max(lineWidth, width);
                height += lineHeight;
                //                }
            }
        }
        //        LogUtils.e("TAG", sizeWidth + "");
        //        LogUtils.e("TAG", width + getPaddingLeft() + getPaddingRight()+"");
        //        LogUtils.e("TAG", height + getPaddingBottom() + getPaddingTop()+"");

        //根据模式判断使用何种宽高
        setMeasuredDimension(modelWidth == MeasureSpec.EXACTLY ? sizeWidth : width + getPaddingLeft() + getPaddingRight(), modelHeight == MeasureSpec.EXACTLY ? sizeHeight : height + getPaddingBottom() + getPaddingTop());
    ...
    protected LayoutParams generateDefaultLayoutParams() {
        ...
        layoutParams.leftMargin = DimensionPixelUtil.dip2px(getContext(), 8);
        return layoutParams;
    }
}