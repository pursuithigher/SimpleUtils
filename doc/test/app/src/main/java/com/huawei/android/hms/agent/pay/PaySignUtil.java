...
public final class PaySignUtil {
    ...
    /**
     * 计算支付请求的签名
     * 重要：此方法仅供单机应用使用。有自己服务器的应用，请在服务器端完成支付的签名，私钥要注意保存避免泄露
     *
     * @param request 支付请求
     * @param privKey 签名所需要的私钥
     * @return string
     */
    public static String calculateSignString(PayReq request, String privKey) {
        ...
        String noSignStr = getNoSign(params, false);
        return rsaSign(noSignStr, privKey);
    }
    ...
    public static String calculateSignString(OrderRequest request, String privKey) {
        ...
        // 必选
        paramcheck.put("merchantId", request.getMerchantId());
        paramcheck.put("requestId", request.getRequestId());
        paramcheck.put("keyType", request.getKeyType());
        paramcheck.put("time", request.getTime());

        String noSignStr = getNoSign(paramcheck, false);
        return rsaSign(noSignStr, privKey);
    }
