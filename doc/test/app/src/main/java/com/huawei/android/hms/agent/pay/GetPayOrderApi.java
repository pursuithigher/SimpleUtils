...
public class GetPayOrderApi extends BaseApiAgent {
    ...
    private void onCheckOrderResult(int retCode, OrderResult checkPayResult) {
        HMSAgentLog.d("callback=" + handler + " retCode=" + retCode + "  checkPayResult=" + checkPayResult);
        if (handler != null) {
            handler.onResult(retCode, checkPayResult);
            handler = null;
        }
     ...
     * @param handler1 查询订单结果回调
     */
    public void getOrderDetail(OrderRequest request, GetOrderHandler handler1) {
        HMSAgentLog.d("getOrderDetail:request=" + request + "  handler=" + handler1);
