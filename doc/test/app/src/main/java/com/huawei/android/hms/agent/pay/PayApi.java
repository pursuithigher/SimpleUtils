...
public final class PayApi extends BaseApiAgent {
    ...
    Status getWaitPayStatus() {
        HMSAgentLog.d("getWaitPayStatus=" + statusForPay);
        return statusForPay;
    }
    ...
    void onPayEnd(int rstCode, PayResultInfo payInfo) {
        HMSAgentLog.d("callback=" + handler + " retCode=" + rstCode + "  payInfo=" + payInfo);
        if (handler != null) {
            handler.onResult(rstCode, payInfo);
            handler = null;
        }
        statusForPay = null;
        payReq = null;
     ...
     *
     * @param requ       支付请求类
     * @param payHandler 支付结果回调
     */
    public void pay(PayReq requ, final PayHandler payHandler) {
        HMSAgentLog.d("pay:requ=" + requ + "  handler=" + payHandler);
